...
class AdversarialAnonymizer:
    ...
    def _create_inference_prompt(self, text: str, target_attributes: List[str]) -> str:
        ...
        """
        # TAB category description mapping (based on TAB guidelines, applicant-focused version)
        attribute_str_map = {
        ...
        # Special instructions (based on TAB guidelines Examples section, applicant-focused context)
        special_notes = {
            ...
            "code": "Look for the applicant's ID-number, case number, application number (e.g., 'no. 42552/98').",
            "loc": "Include number when part of entity name (e.g., 'Pilestredet 48'). Include airport, hospital, shop, road where the applicant resides or was located.",
        ...
        prompt = f"""Let us play a guessing game. The following is a legal case document. Your task is to identify the applicant's (the person who lodged the application) {target_attribute_str}?
...
Example: 'my husband and I' -> 'I' is also valid as it only removes information."""
            header = "Below I give you a legal case document as well as personal inferences made about the applicant that can be made from the text. Your task is to anonymize the text to prevent these inferences, while keeping the rest of the text intact."
